...
void escribirArchivoEmpresas(Empresa e[], int cantEmpresas, float importeTotal[][3], int cantidadVentasPorEmpresa[]) 
{
    ...
    if (archivo !=NULL) 
    {
        for (int i = 0; i < cantEmpresas; i++) 
        {
            ...
            fwrite(&e[i].codEmpresa, sizeof(e[i].codEmpresa), 1, archivo);
            fwrite(e[i].nomEmpresa, sizeof(e[i].nomEmpresa), 1, archivo);
            fwrite(&totalImporte, sizeof(totalImporte), 1, archivo);
            fwrite(&cantidadVentasPorEmpresa[i], sizeof(cantidadVentasPorEmpresa[i]), 1, archivo);
        }
    fclose(archivo);
    }
    ...
    {
        cout<<"error al abrir el archivo ImporteEmpresas.dat"<<endl;
    } 
}
...
void escribirArchivoSinAdhesion(Venta v[], int cantVentas, Empresa e[], int cantEmpresas) 
{
        ...
        for (int i = 0; i < cantVentas; i++) 
    {
        ...
        {
            fwrite(&v[i], sizeof(v[i]), 1, archivo);
        }
    }
    fclose(archivo);
    }
